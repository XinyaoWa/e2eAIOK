...
class FinetuneArguments:
    ...
    input_sentence: Optional[str] = field(
        default="",
        metadata={"help": "generate output for input sentence"},
    )
    output_length_limit: int = field(
        default=20,
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        model_dtype = torch.float16 if training_args.fp16 else None
        model = AutoModelForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
            config=config,
            cache_dir=model_args.cache_dir,
            load_in_8bit=finetune_args.load_in_8bit, 
            revision=model_args.model_revision,
            use_auth_token=True if model_args.use_auth_token else None,
            trust_remote_code=True if model_args.trust_remote_code else None,
    ...
    if training_args.do_predict:
        logger.info("*** predict ***")
        inputs = tokenizer([finetune_args.input_sentence], return_tensors="pt").to(training_args.device)
        generation_output = model.generate(**inputs, max_new_tokens=finetune_args.output_length_limit, return_dict_in_generate=True, output_scores=True)
        s = generation_output.sequences[0]
        output = tokenizer.decode(s)
        print(f"Input: {finetune_args.input_sentence}")
